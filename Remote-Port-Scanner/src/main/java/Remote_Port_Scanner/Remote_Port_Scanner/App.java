package Remote_Port_Scanner.Remote_Port_Scanner;
...
public class App {
	public static void main(String[] args) {
		...
		System.out.print("Please enter the target ip address: "); // ask user to
																	// enter the
																	// ip
																	// address
		...
		while (!isValid) {
			try {
				System.out.print("Please enter the first port: "); // ask user
																	// to type
																	// the first
																	// port
				String portString = reader.readLine(); // read the port number
				fromPort = Integer.parseInt(portString); // try to convert
															// string to integer
				if (fromPort >= 0 && fromPort <= 65536) // verify that port is
														// in the valid range
				{
					isValid = true; // set variable to true and exit the while
									// loop
				} else {
					System.out.println("Invalid port! Port range is: 0 - 65536"); // inform
																					// user
																					// about
																					// the
																					// valid
				...
				System.out.println("Please insert a number!"); // if the user
																// does not
																// enter a
																// number
			} catch (Exception e) // error when trying to read user input
		...
		isValid = false; // reinitialize the boolean value to false in order to
							// start a new validation process

		// as long as isValid is false, try to obtain a valid port number
		while (!isValid) {
			try {
				System.out.print("Please enter the last port: "); // ask user to
																	// type the
																	// last port
				String portString = reader.readLine(); // read the port number
				toPort = Integer.parseInt(portString); // try to convert string
														// to integer
				if (toPort >= 0 && toPort <= 65536) // verify that port is in
													// the valid range
				{
					if (toPort >= fromPort) // verify if the toPort is greated
											// than fromPort
					{
						isValid = true; // set the boolean value to true and
										// exit the while loop
					}
				} else {
					System.out.println("Invalid port! Port range is: 0 - 65536"); // inform
		...
		{
			try {
				Socket socket = new Socket(targetIp, port); // try to open on
															// socket connection
															// using the
															// specified ip
															// address and the
															// test port number
				System.out.println("Port " + port + " is in listening state!"); // print
																				// listening
																				// port
				socket.close();
			} catch (UnknownHostException e1) // catch block is executed if an
												// invalid host was entered
			{
				System.out.println("Unknown host exception " + e1.toString());
			} catch (IOException e2) // execute when the port is already opened
			{
				System.out.println("Port " + port + " is not opened!"); // print
																		// what
																		// port
																		// is
																		// opened
